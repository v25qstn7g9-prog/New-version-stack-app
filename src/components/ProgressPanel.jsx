      ...
      <Panel title="計畫進度追蹤">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div className="rounded-xl p-3 flex flex-col justify-between" style={{ background: COLORS.bg, border: `1px solid ${COLORS.panelBorder}` }}>
            <span className="text-[11px]" style={{ color: COLORS.sub }}>成本進度 vs 已執行月數</span>
            <span className="text-lg font-bold mono mt-1"
              ...
              {costLeadMonths == null ? "—" : `${costLeadMonths >= 0 ? "+" : ""}${costLeadMonths.toFixed(1)} 個月`}
            </span>
            <span className="text-[10px] mt-1" style={{ color: COLORS.sub }}>
              {costLeadMonths == null ? "" : (costLeadMonths >= 0 ? "投入節奏領先計畫" : "投入節奏落後計畫")}
            </span>
          </div>
          <div className="rounded-xl p-3 flex flex-col justify-between" style={{ background: COLORS.bg, border: `1px solid ${COLORS.panelBorder}` }}>
            <span className="text-[11px]" style={{ color: COLORS.sub }}>資產進度 vs 已執行月數</span>
            <span className="text-lg font-bold mono mt-1"
              style={{ color: assetLeadMonths == null ? COLORS.text : (assetLeadMonths >= 0 ? COLORS.gain : COLORS.loss) }}>
              {assetLeadMonths == null ? "—" : `${assetLeadMonths >= 0 ? "+" : ""}${assetLeadMonths.toFixed(1)} 個月`}
            </span>
            <span className="text-[10px] mt-1" style={{ color: COLORS.sub }}>
              {assetLeadMonths == null ? "" : (assetLeadMonths >= 0 ? "資產成長領先假設報酬率" : "資產成長落後假設報酬率")}
            </span>
          </div>
        </div>
