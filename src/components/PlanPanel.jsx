...
function PlanPanel({ planItems, setPlanItems, planTotal, goal, setGoal, exportBackup, importBackup, exportExcel, lastBackupAt, backupAgeDays, backupOverdue, aiFallbackConfig, setAiFallbackConfig, healthCards, healthCardBusyId, respondHealthCard }) {
  ...
  const testExternalAiFallback = async () => {
    ...
    if (!healthUrl) {
      setFallbackTest({ status: "error", message: t("網址格式不正確；可直接貼 Vercel 專案網址") });
      return;
    }
    setFallbackTest({ status: "loading", message: t("測試中…") });
    try {
      const headers = {};
      ...
      setFallbackTest({ status: "ok", message: t("備援後端正常{model}", { model }) });
    } catch (e) {
      setFallbackTest({ status: "error", message: e?.message || t("備援測試失敗") });
    }
  };
  const add = () => {
  ...
  return (
    <div className="space-y-4">
      <Panel title={t("退休目標設定")}>
        <div className="grid grid-cols-2 gap-2">
          <Field label={t("目標金額 (NTD)")}><input type="number" value={goal.targetAmount}
            onChange={(e) => setGoal({ ...goal, targetAmount: Number(e.target.value) })} className="input" /></Field>
          <Field label={t("目標年份")}><input type="number" value={goal.targetYear}
            onChange={(e) => setGoal({ ...goal, targetYear: Number(e.target.value) })} className="input" /></Field>
        </div>
      </Panel>

      <Panel title={t("每月定期定額（合計 NT$ {amount}）", { amount: nf(planTotal) })}>
        <div className="space-y-2">
          ...
          {planItems.length === 0 && <Empty text={t("尚未設定定期定額配置")} />}
        </div>
      </Panel>

      <Panel title={t("新增定期定額項目")}>
        <div className="grid grid-cols-2 gap-2">
          <Field label={t("標的")}><input value={form.symbol}
            onChange={(e) => setForm({ ...form, symbol: e.target.value })} placeholder="0050" className="input" /></Field>
          <Field label={t("月投入金額")}><input type="number" value={form.amount}
        ...
        <AddButton onClick={add} label={t("新增項目")} />
      </Panel>
        ...
        <Panel title={t("系統健康")}>
          {healthCards.map((card) => (
            <div key={card.id} className="mb-3 rounded-xl p-3" style={{ background: COLORS.bg, border: `1px solid ${COLORS.panelBorder}` }}>
              <div className="text-[12px] leading-5" style={{ color: COLORS.text }}>{t(card.summary)}</div>
              {card.occurrences > 1 && (
                <div className="mt-1 text-[10px]" style={{ color: COLORS.sub }}>{t("已連續偵測到 {n} 次，最後一次：{time}", { n: card.occurrences, time: card.lastSeenAt })}</div>
              )}
              {card.fixType === "wait" && (
                <div className="mt-1 text-[10px]" style={{ color: COLORS.sub }}>{t("建議：不用做什麼，等它自動恢復即可。")}</div>
              )}
              {card.fixType === "code" && (
                <div className="mt-1 text-[10px]" style={{ color: COLORS.sub }}>{t("建議：這個需要改程式碼重新部署，按「確定」只會把這張卡片標記為已處理，不會自動改程式。")}</div>
              )}
      ...
      <Panel title={t("AI 雙層備援（自動）")}>
        <div className="text-[11px] leading-5" style={{ color: COLORS.sub }}>
          {t("主 AI：")}<span style={{ color: COLORS.gold }}>Cloudflare GPT-OSS</span>{t("。如果遇到額度用完、429、逾時或服務錯誤，會由同一個 Cloudflare Worker ")}<span style={{ color: COLORS.gold }}>{t("直接切到 Gemini 3.5 Flash-Lite")}</span>{t("，不再需要 Vercel，也不會把 Gemini API Key 放在手機。")}
        </div>
        <div className="mt-2 rounded-xl p-3 text-[11px] leading-5" style={{ background: COLORS.bg, border: `1px solid ${COLORS.panelBorder}`, color: COLORS.sub }}>
          ...
          <div><span style={{ color: COLORS.gold }}>↓ {t("失敗自動切換")}</span></div>
          <div><span style={{ color: COLORS.gain }}>②</span> {t("Gemini 3.5 Flash-Lite（Worker 伺服器端）")}</div>
          <div><span style={{ color: COLORS.sub }}>↓ {t("如果 Gemini 也失敗，才顯示真正錯誤，不會假裝成功")}</span></div>
        </div>
        <label className="mt-3 flex items-start gap-2 text-[11px] leading-5" style={{ color: COLORS.sub }}>
          <input type="checkbox" checked={aiFallbackConfig?.allowPrivate === true}
            ...
            style={{ marginTop: 3 }} />
          <span>{t("允許 Gemini 備援讀取 App 的持股／成本／資產資料。關閉時，公開資訊、聊天、新聞與即時股價仍可備援；需要私人資產資料時，Gemini 會改用唯讀工具請 App 查詢，不會把整份私人 context 送出去。")}</span>
        </label>
      </Panel>

      <Panel title={t("資料備份與還原")}>
        <div className="rounded-xl p-3 flex gap-2 items-start"
          ...
          <div className="text-[11px] leading-5" style={{ color: COLORS.sub }}>
            {t("JSON 用來完整備份／還原 App；Excel 用來保存、檢視與分析投資歷史。重要修改後建議兩種都留一份。")}
          </div>
          <div className="ml-auto text-right text-[10px] leading-4" style={{ color: backupOverdue ? COLORS.gold : COLORS.sub }}>
            {lastBackupAt
              ? <>{t("上次 JSON 備份")}<br />{new Date(lastBackupAt).toLocaleDateString(lang === "en" ? "en-US" : "zh-TW")}（{t("{n} 天前", { n: backupAgeDays })}）</>
              : <>{t("上次 JSON 備份")}<br />{t("尚未記錄")}</>}
          </div>
        </div>
